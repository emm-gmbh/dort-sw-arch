...
#include "timers.h"
#include "semphr.h"
extern void DortMainTask(void *pvParameters);
...
#ifndef RRDU_VALIDATION_DEMO
#define RRDU_VALIDATION_DEMO 0
#endif

#if RRDU_VALIDATION_DEMO
    bringup_validation_run_once();
    printf_to(UART_APP, "\r\n[BRINGUP-VALIDATION] result=%d state=%d trace_len=%u\r\n",
             (int)g_bringup_validation_result,
             (int)g_bringup_validation_state,
             (unsigned)g_bringup_trace_len);
#endif

    rtos_result = xTaskCreate( DortMainTask, "DORT", configMINIMAL_STACK_SIZE, NULL, PRIMARY_PRIORITY, NULL );
    if(1 != rtos_result)
    {
        int ix;
        for(;;)
